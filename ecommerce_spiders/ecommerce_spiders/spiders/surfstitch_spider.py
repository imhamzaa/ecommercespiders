...
class SurfStitch(CrawlSpider):
    ...
    def parse_product(self, response):
        ...
        if unselected_color_urls:
            garment['requests'] += self.color_request(response, garment, unselected_color_urls)
            result = self.request_or_garment(garment)
        return result if result else self.request_or_garment(garment)

    def color_request(self, response, garment, unselected_color_urls):
        requests = []
        for url in unselected_color_urls:
            requests.append(Request(url=response.urljoin(url), callback=self.parse_color, meta={'garment': garment}))
        return requests
    ...
    def size_or_sku(self, response):
        ...
        if unselected_size_urls:
            response.meta['garment']['requests'] += self.size_request(response, unselected_size_urls)
    ...
    def update_sku(self, response):
        ...
        if selected_size:
            sku_id = f'{selected_color.strip()}_{selected_size.strip()}'
        else:
            sku_id = f'{selected_color.strip()}_ONE_SIZE'
